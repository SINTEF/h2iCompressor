...
Cm1i = np.arange(10, 200.5, 0.5)       # Inlet Absolute meridional velocity [m/s]
...
# ------------ New scheme ------------
rhDivr1=0.65                                                    # Ratio commonly given
#rt1i = (A1i / (math.pi * (1 - (rh) ** 2))) ** 0.5              # Inlet tip radius [m]
rt1i = (A1i / (math.pi* (1 - rhDivr1**2) )) ** 0.5              # Inlet tip radius [m]                           eqn (52) rapport
rh = rhDivr1*rt1i
U2Crit = np.sqrt(2*steelTensileStrength/steelDensity)           # Flat disc, thick disk and impeller will break later
marginU2 = 0.9                                                  # could probably multiply with number greater than 0, 1.5 at least probably
U2 = marginU2*U2Crit                     
r1Dr2 = np.arange(0.8, 0.3, -0.05)       # r1 divided by r2 ,making r2 increase through array progression
r1Dr2 = r1Dr2.reshape(-1, 1)
r2 = rt1i/r1Dr2
U1Dr1 = U2/r2
U1 = r1Dr2*U2
N = 60*U1/(2*np.pi *rt1i)
...
W1ti = (Cm1i ** 2 + (U1 - Ctheta1i) ** 2) ** 0.5        # Todo    : enig i denne linjen, ikke den over
W1Dr1 = ((Cm1i/rt1i) ** 2 + (U1Dr1 - Ctheta1i/rt1i) ** 2) ** 0.5        # Todo    : enig i denne linjen, ikke den over

### kvalitetssikret hittil => fortsett onsdag

# Todo: Hvorfor ønsker man å minimere W1ti??? => efficiency

"""Problemstilling: r1 > rt2, må iterere å finne den indexen som gir minst relativ hastighet samtidig som r2t>r1
    EDIT: r1 er alltid større enn rt2  => PROBLEM"""



indice_min = np.argmin(W1ti, axis=1)
rt1 = rt1i[indice_min]           # Replaced by prior loop 
Ctheta1 = Ctheta1i[indice_min]     # Get the value of Ctheta1 for minimal value of W1t
Ctheta1 = Ctheta1.reshape(-1, 1)    # Get the value of Ctheta1 for minimal value of W1t
C1 = C1i[indice_min]        
T1 = T1i[indice_min]
M1 = M1i[indice_min]
P1 = P1i[indice_min]
...
# U1t = U1ti[indice_min]
Cm1 = Cm1i[indice_min]
Cm1 = Cm1.reshape(-1, 1)    # Get the value of Ctheta1 for minimal value of W1t
# Ctheta1 = np.squeeze(Ctheta1) 
beta1 = np.rad2deg(np.arctan((U1 - Ctheta1) / Cm1))      # Inlet relative velocity angle [deg]
W1t = W1ti[indice_min]
...
"""
W1tIter = W1tIterate(Cm1i, U1, Ctheta1i)      # Get W1t as a function of Cm1
plt.figure('W1 plot')
...
breakCheck = False
"""
Cm11 = np.arange(10, 200, 2)       # Inlet Absolute meridional velocity [m/s]
r1Dr2 = np.arange(0.8, 0.3, -0.05)       # r1 divided by r2 ,making r2 increase through array progression
r1Dr2 = r1Dr2.reshape(-1, 1) 
...
U2 = 0.9*U2Crit
U1 = r1Dr2*U2
Ctheta11 = Cm11 * math.tan(math.radians(alpha1))
W1t1 = (Cm11 ** 2 + (U1 - Ctheta1) ** 2) ** 0.5        # Todo    : enig i denne linjen, ikke den over
indexMins = np.argmin(W1t1, axis=1)
    ...
    for iz in range(len(ZBarr)):
        ...
        for ib in range(len(beta2bArr)):
            ...
            while ( (trueFalseMat[ib, iz][1] == False) and (etaStage < etaUpperLimit and etaStage > etaLowerLimit) ):
                ...
                U2 = 0.8*U2Crit                     # need some margin
                U1 = U2*r1Dr2

                r1 = U1*(1/U1divr2)*r1Dr2
                r2 = r1*(1/r1Dr2)

                A11 = mdot / (rho1 * Cm11 * (1 - B1))                      # Inlet flow area [m^2]                          Continuity                            

                rh1 = np.sqrt(r1**2 - (A11/np.pi))
                D2 = 2*r2
                D1 = 2*r1
                N1 = 30*U1/(np.pi*r1)       # N1 and N2 should be equal, U1/r1=U2/r2
                N2 = 30*U2/(np.pi*r2)       # N1 and N2 should be equal     => debug
                N = N1
                Ncrit = 30*U2Crit/(np.pi*r2)
