...
Cp_air = 1006       # Specific heat at constant pressure of air [J/kg/K]. MSG: Make fluid specific properties belong to different classes                          DENNE MED ENHET J/KG*K
Cp_H2 = 14246      # Specific heat at constant pressure of hydrogen [J/kg/K]. MSG: Make fluid specific properties belong to different classes                     DENNE MED ENHET KJ/KG*K => MÅ VÆRE J/KG*K
Cp = Cp_H2         # Set specific heat at constant pressure [J/kg/K]
...
R = R_uni / M_H2   # Set specific gas constant [J/kg/K]
...
M1i = C1i / ((k * R * T1i) ** 0.5)                  # Inlet Mach number [-]         # not used
...
M1 = M1i[np.argmin(W1ti)]       # not used
...
while abs(Pr_iterate - Pr) / Pr > pressure_ratio_tolerance:
    ...
    if rt1 / (0.5 * D2) < math.exp(- 8.16 * math.cos(beta2b_iterate) / ZB_iterate):     # Check for sigma validity. MSG: Understand this condition. Is the condition very loose? 
        print('sigma valid', 'LHS: ', rt1 / (0.5 * D2), '    RHS: ', math.exp(- 8.16 * math.cos(beta2b_iterate) / ZB_iterate))
    else:   
        ZB_iterate -= 1         
    ...
    eta_iterate += 0.01
    if round(eta_previous, 5) == round(eta_iterate, 5):
        print('eta stagnating at', round(eta_iterate, 5), ' and Pr=', round(Pr_iterate, 4), '. exit velocity U2= ', U2 , '\n')
        break
