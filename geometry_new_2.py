...
def iterate_blade_number_and_blade_angle(Compressor, InletConditions, Fluid, IterationMatrix):
    ...
    for iz in range(len(IterationMatrix.ZBarr)):
        for ib in range(len(IterationMatrix.beta2BArr)):
            ...
            etaStage = 0.7 #Compressor.etaStage0                     # resetting the guessed efficiency for each new combination of blade number and blade angle
            Pr_stage = 1.1
            eta_max = 0.80
            while etaStage < eta_max- 0.01 or etaStage > eta_max + 0.01: 
                
                dh0s = ((Fluid.k * Fluid.R * InletConditions.T00) / (Fluid.k - 1)) * (Pr_stage ** ((Fluid.k - 1) / Fluid.k) - 1)       
                Wx = dh0s / eta_max                         # Specific work [J/kg/K]
                ...
                Pr_stage = (1 + etaStage * (T02m / InletConditions.T00 - 1)) ** (Fluid.k / (Fluid.k - 1))  #p5 / InletConditions.P00
                print('etaStage', etaStage)
                print('Pr_stage', Pr_stage)
                print('p5 / P00', p5 / InletConditions.P00)
    ...
    if countTrue == 0:
        ...
        print('eta', IterationMatrix.etaMat[np.where(IterationMatrix.beta2BArr == Compressor.bladeAngle)[0][0]][np.where(IterationMatrix.ZBarr == Compressor.bladeNumber)[0][0]])
