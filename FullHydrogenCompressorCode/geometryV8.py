...
rt1i = (A1i / (math.pi* (1 - settingsOffDesign.rhDivr1**2) )) ** 0.5             # Inducer tip radius [m]
...
from isentropic relationsettingsOffDesign. Further, geometrical relations are applied. - Petter """
...
dh0s = ( (settingsOffDesign.k * settingsOffDesign.R * settingsOffDesign.T00) / (settingsOffDesign.k - 1) ) * (settingsOffDesign.Pr **((settingsOffDesign.k - 1)/ settingsOffDesign.k) - 1)                     # Specific isentropic compression enthalpy, constant value [kJ/kg/K]
...
omegaCritMultr1 = np.sqrt(3*settingsOffDesign.impellerTensileStrength/ settingsOffDesign.impellerDensity )/(1/settingsOffDesign.r1Divr2)
...
while (U2) >= U2Crit and Ndes > 0:
    # U2omega
    # U1divr2 = 2*np.pi * settingsOffDesign.r1Divr2 * Ndes / 60     
...
for iN in range(1): # To be iterated for rpm N
    ...
    for iz in range(len(ZBarr)):
        ...
        for ib in range(len(beta2BArr)):
            ...
            etaStage = settingsOffDesign.etaStage0
            trueFalse2 = False
...
# show = settingsOffDesign.showGeometryPlots
# plt.show()
