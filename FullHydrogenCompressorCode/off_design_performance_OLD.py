...
for i in range(0, len(V0DivVcr)):
    ...
    # Does not make a big difference but still
...
def Densityiteration(rho2o):
    ...
    density.
    """
    Vm2m = mdoto / (math.pi * rho2o * (2*geometryV8.r2) * geometryV8.b2)                        # (B49) Meridional component of exit absolute velocity [m/s]  , MASS BALANCE                
    ...
    HydDiaDivExitDia = 1 / (ZB / (math.pi * math.cos(math.radians(beta2B)) + geometryV8.D2 / geometryV8.b2)) + (2  * geometryV8.r1 / geometryV8.D2) / (2 / (1 - settingsOffDesign.k) + 2 * ZB / (math.pi * (1 + settingsOffDesign.k)) * math.sqrt(1 + (math.tan(math.radians(geometryV8.beta1) **2 ) * (1 + settingsOffDesign.k ** 2 / 2))))  # Ratio of hydraulic diameter and exit diameter [-]
...
for i in range(0, len(V0DivVcr)):
    ...
    Pro.append(P3o[i] / settingsOffDesign.P00)
...
print('RPM: ' + str(Ndes))
print('r1: ' + str(geometryV8.r1))
print('r2: ' + str(geometryV8.r2))
# plt.show()
