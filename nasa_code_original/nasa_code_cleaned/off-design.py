...
VOVCR = np.arange(0.1, 0.375, 0.025)  # Compressor inlet absolute critical velocity ratio [-]
...
T2oestabs = (dhest / (Cp * T00) + 1) * T00  # (B47) Estimate of the off design impeller exit total temperature
rho2o = rho1 * (T2oestabs / T00)**(1 / (k - 1))  # (B48) Off design impeller exit density [kg/m^3]

# The Densityiteration function takes an initial guess of the impeller outlet density using Equation B48 above.
# It then uses this initial guess to calculate a series of velocities, temperatures and enthalpies corresponding to this initial guess before re-calculating the density.
def Densityiteration(rho2o):
    ...
    Df = (1 - W2 / W1to + (kBL * qaero) / ((W1to / U2) * ((ZB / math.pi) * (12 * rt1 / D2) + 2 * 2 * rt1 / D2)))  # (B59) Diffusion factor [-]
    ...
    D1rms = math.sqrt((((2 * rt1)**2) + ((2 * rh1)**2)) / 2)  # Root mean square of the diameter [m]
...
# The Density function selects the value corresponding to the variables listed below when the output density from the DensityIteration function is within 0.1% of the input
def Density():
    rhoinit = rho2o
    RHO = []
    T2o = []
...
def pressure_ratio1(mdot, eta, comp_power, P, T1, y, cp):
    ...
    print(T1)
    print(y)
    return (eta * comp_power / (mdot[1:] * cp * T1) + 1)**(y / (y - 1))
...
for i in range(1):
    ...
    plt.plot(mdot[1:], Pr, 'g-', label="100kW", linewidth=1.25)
plt.legend()
plt.show()