...
r1rms = math.sqrt((geometry.rt1 ** 2 + geometry.rh1 ** 2) / 2)  # Root mean square of the hub and tip inlet radius [m]

h0 = r1rms - geometry.rh1            # (B4) Spacing for numerical integration [-]
h1 = geometry.rt1 - r1rms            # (B5) Spacing for numerical integration [-]
...
rho1h = rho0 * (1 - (Cm1h ** 2 / (2 * geometry.Cp * geometry.T00))) ** (1 / (geometry.k - 1))      # Inlet density of the air at the hub [kg/m^3]
rho1rms = rho0 * (1 - (Cm1rms ** 2 / (2 * geometry.Cp * geometry.T00))) ** (1 / (geometry.k - 1))  # Root mean square density of the air [kg/m^2]
rho1t = rho0 * (1 - (Cm1t ** 2 / (2 * geometry.Cp * geometry.T00))) ** (1 / (geometry.k - 1))      # Inlet density of the air at the blade tip [kg/m^3]
mdoto = 2 * math.pi * (((h0 + h1) / 6) * ((2 - (h1 / h0)) * (rho1h * geometry.rh1 * Cm1hn) + ((h0 + h1) ** 2 / (h0 * h1)) * (rho1rms * r1rms * Cm1rmsn) + (2 - (h0 / h1)) * (rho1t * geometry.rt1 * Cm1tn)))      # (B7) Off Design point mass flow rate [kg/s]
U1to = math.pi * No * Ndes * 2 * geometry.rt1 / 60       # Off design blade tip velocity [m/s]
...
BBF = 1 - (geometry.ZB * tu) / (2 * math.pi * r1rms)     # (41) Blade Blockage Factor [-]
eps = []
betaopt = []
WL = []
dhinc = []
...
T2oestabs = (dhest / (geometry.Cp * geometry.T00) + 1) * geometry.T00          # (B47) Estimate of the off design impeller exit total temperature [K]
rho2o = geometry.rho1 * (T2oestabs / geometry.T00) ** (1 / (geometry.k - 1))   # (B48) Off design impeller exit density [kg/m^3]
...
def Densityiteration(rho2o):
    ...
    density.
    """
    Vm2m = mdoto / (math.pi * rho2o * geometry.D2 * geometry.b2)    # (B49) Meridional component of exit absolute velocity [m/s]
    VSL = U2o * (1 - geometry.sigma)                                # (B51) Slip velocity [m/s]
    ...
    T2o = (T2orel - ((W2 ** 2) / (2 * geometry.Cp)))                # (B55) Off design point exit temperature [K]
    V2 = ((Vm2m ** 2) + (Vtheta2 ** 2)) ** 0.5                      # (B56) Off design point absolute exit velocity [m/s]
    T2oabs = (T2o + (V2 ** 2) / (2 * geometry.Cp))                  # (B57) Off design point exit temperature in the absolute reference frame [K]
    dhaero = (geometry.Cp * geometry.T00 * (T2oabs / geometry.T00 - 1))        # (B61) Aerodynamic enthalpy rise [J/kg]
    qaero = dhaero / (U2o ** 2)                                     # (B60) Dimensionless actual head [-]
    Df = (1 - W2 / W1to + (kBL * qaero) / ((W1to / geometry.U2) * ((geometry.ZB / math.pi) * (1 - 2 * geometry.rt1 / geometry.D2) + 2 * 2 * geometry.rt1 / geometry.D2)))     # (B59)Diffusion factor [-]
    ...
    D1rms = math.sqrt(((( 2 * geometry.rt1) ** 2) + ((2 * geometry.rh1) ** 2)) / 2)         # Rootmean square of the diameter [m]
    Lendia = 0.5 * (1 - (D1rms / 0.3048)) / (math.cos(math.radians(geometry.beta2b)))       # (B65) Blade length to diameter ratio [-]
    HYDdia = 1 / (geometry.ZB / (math.pi * math.cos(math.radians(geometry.beta2b)) + geometry.D2 / geometry.b2)) + (2  * geometry.rt1 / geometry.D2) / (2 / (1 - geometry.k) + 2 * geometry.ZB / (math.pi * (1 + geometry.k)) * math.sqrt(1 + (math.tan(math.radians(geometry.beta1) **2 ) * (1 + geometry.k ** 2 / 2))))  # Ratio of hydraulic diameter and exit diameter [-]
    WRelExt = 0.5 * ((Cm1rms / U2o) ** 2 + (D1rms / geometry.D2) ** 2 + (W2 / W1to) ** 2 * ((Cm1rms / U2o) ** 2 + (2 * geometry.rt1 / geometry.D2) **2 ))      # (B67) Ratio of mean relative velocity and impeller exit velocity^2 [-]
    ...
    P2oabs = (P1arms * (etaR * dhaero / (geometry.Cp * geometry.T00) + 1) ** (geometry.k / (geometry.k - 1)))       # (B70) Iteration of the off design exit absolute pressure [Pa]
    for Temp in T2o:
    ...
    P2o = (P2oabs / ((T2oabs / T2o) ** (geometry.k / (geometry.k - 1))))    # (B71) Iteration of the off design exit pressure [Pa]      
    rho2oit = P2o / (geometry.R * T2o)                                      # (B72) Iteration of the off design exit density [kg/m^3]
...
for i in range(0, len(VOVCR)):
    ...
    C3o.append(C2o[i] / geometry.AR)
    P03o.append(P3o[i] + 0.5 * rho2[i] * C3o[i] ** 2)
    M3o.append(C3o[i] / (math.sqrt(geometry.k * geometry.R * T2oabs[i])))
    P3oabs.append(P3o[i] * (1 + (geometry.k - 1) / 2 * M3o[i] ** 2) ** (geometry.k / (geometry.k - 1)))    # (B85) Absolute diffuser throat pressure [Pa]
    dhVLD.append(geometry.Cp * T2oabs[i] * ((P3o[i] / P3oabs[i]) ** ((geometry.k - 1) / geometry.k) - (P3o[i] / P2oabs[i]) ** ((geometry.k - 1) / geometry.k)))  # (B86) Vaneless diffuser loss [kJ/kg]
